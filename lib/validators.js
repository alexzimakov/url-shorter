...
/**
 * Returns validator for checking if user is exists.
 * 
 * @param {String} field 
 * @returns {Function}
 */
function isUserExists(field) {
  return value => User.objects({ filter: { [field]: value } })
    .then((users) => {
      if (users.length > 0) {
        throw new Error(`User with ${field} is already exists.`);
      }
